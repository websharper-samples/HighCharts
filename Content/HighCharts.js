...
(function()
{
 var Global=this,Runtime=this.IntelliFactory.Runtime,jQuery,Site,Samples,List,ChartControl,Set,MapControl,StockControl,T,Html,Client,Tags,Client1,Arrays,Builder,Operators,Sample,document,Operators1,Unchecked,Attr,EventsPervasives,Seq;
 Runtime.Define(Global,{
  Site:{
   ChartControl:{
    Main:function(el)
    {
     var returnVal,returnVal1,returnVal2,returnVal3,returnVal4,returnVal5;
     returnVal={};
    ...
    {
     return Samples.Build().Id("Highcharts").FileName("Highchart.fs").Keywords(List.ofArray(["highcharts","charts"])).Render(function(el)
     {
      return ChartControl.Main(el);
     }).Create();
    })
   },
   Client:{
    ...
    {
     var _op_BangPlus_9;
     _op_BangPlus_9=function(x)
     {
      return Set.Singleton(x);
     };
     return Set.Create(List.ofArray([_op_BangPlus_9(MapControl.Sample()),_op_BangPlus_9(ChartControl.Sample()),_op_BangPlus_9(StockControl.Sample())]));
    }),
    Main:Runtime.Field(function()
    {
     ...
     StockControl.Main(Tags.Tags().NewTag("div",arg10).Dom);
   ...
   MapControl:{
    Main:function(el)
    {
     var returnVal,returnVal1,returnVal2,returnVal3,returnVal4,returnVal5;
     returnVal={};
     returnVal.spacingBottom=20;
     returnVal1={};
     returnVal1.text="Europe time zones";
     returnVal2={};
     returnVal2.enabled=true;
     returnVal3={};
     returnVal4={};
     ...
     returnVal5={};
     returnVal5.headerFormat="";
     returnVal5.pointFormat="{point.name}: <b>{series.name}</b>";
     returnVal3.map={
      allAreas:false,
      joinBy:["iso-a2","code"],
      dataLabels:returnVal4,
      tooltip:returnVal5
     };
     jQuery(el).highcharts("Map",{
      ...
      title:returnVal1,
      legend:returnVal2,
      plotOptions:returnVal3,
      series:[{
       name:"UTC",
       mapData:MapControl.map(),
       data:Arrays.map(function(e)
       {
        return{
         code:e
        };
       ...
       },["NO","SE","DK","DE","NL","BE","LU","ES","FR","PL","CZ","AT","CH","LI","SK","HU","SI","IT","SM","HR","BA","YF","ME","AL","MK"])
      },{
       name:"UTC +2",
       mapData:MapControl.map(),
       data:Arrays.map(function(e)
    ...
    Sample:Runtime.Field(function()
    {
     return Samples.Build().Id("Highmaps").FileName("Highmap.fs").Keywords(List.ofArray(["highmaps","charts"])).Render(function(el)
     {
      return MapControl.Main(el);
     }).Create();
    }),
    map:Runtime.Field(function()
    {
     return Global.Highcharts.maps["custom/europe"];
    })
   },
   Samples:{
    Build:function()
    ...
    Sample:Runtime.Class({
     Show:function()
     {
      ...
      url="http://github.com/intellifactory/websharper.highcharts/blob/master/Site/"+this.FileName;
      ...
      arg10=List.ofArray([x,Operators1.add(Tags.Tags().NewTag("a",arg102),List.ofArray([Tags.Tags().text("Source")]))]);
    ...
    Set:Runtime.Class({
     Show:function()
     {
      ...
      return jQuery(function()
      {
       ...
       navs=Operators1.add(Tags.Tags().NewTag("ul",arg10),List.mapi(function(i)
       {
        return function(s1)
        {
         ...
         x=s1.Title;
         arg101=List.ofArray([Operators1.add(Tags.Tags().NewTag("a",arg102),List.ofArray([Tags.Tags().text(x)]))]);
   ...
   StockControl:{
    Main:function(el)
    {
     jQuery.getJSON("http://www.highcharts.com/samples/data/jsonp.php?filename=aapl-c.json&callback=?",function(data)
     {
      var returnVal,returnVal1;
      returnVal={};
      returnVal.text="AAPL stock price";
      returnVal1={};
      returnVal1.selected=1;
      returnVal1.inputEnabled=jQuery(el).width()>480;
      jQuery(el).highcharts("StockChart",{
       title:returnVal,
       rangeSelector:returnVal1,
       series:[{
        name:"AAPL",
        data:data
       }]
    ...
    Sample:Runtime.Field(function()
    {
     return Samples.Build().Id("Highstock").FileName("Highstock.fs").Keywords(List.ofArray(["highstock","charts"])).Render(function(el)
     {
      return StockControl.Main(el);
     }).Create();
    })
   }
  }
 ...
 Runtime.OnInit(function()
 {
  ...
  List=Runtime.Safe(Global.WebSharper.List);
  ChartControl=Runtime.Safe(Site.ChartControl);
  Set=Runtime.Safe(Samples.Set);
  MapControl=Runtime.Safe(Site.MapControl);
  StockControl=Runtime.Safe(Site.StockControl);
  T=Runtime.Safe(List.T);
  Html=Runtime.Safe(Global.WebSharper.Html);
  Client=Runtime.Safe(Html.Client);
